...
export function attachRecorder(site, ctx, log = () => {}) {
  ...
  async function onResp(page, r) {
    try {
      ...
      seen.set(`${req.method()} ${cleanKey(url)}`, { url, method: req.method(), status: r.status(), body, requestBody: req.postData() ?? null, headers, pageUrl: page.url() });
    } catch { /* 忽略单条读取失败 */ }
  }
  const attach = (page) => {
...
function buildSpec(site, c, cfg) {
  ...
  return {
    ...
    url: c.url,
    method: c.method,
    // POST 必须用真实请求体：body 是响应文本（供识别打分用），不能当请求体回放
    body: c.method === 'POST' ? (c.requestBody ?? null) : null,
    headers: c.headers,
