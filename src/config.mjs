...
//  - 阿里: 校招门户迁至 campus-talent.alibaba.com（我的投递 /personal/applications），
//    接口 POST /application/applies 带 _csrf 时效令牌 → 直连不可重放，页面模式回放
...
// 公司别名：邮件关联（maillinks）与状态包（trackerSync）共用的匹配口径（台账/邮件可能用简称）
...
export const SITES = {
  ...
  alibaba: {
    label: '阿里巴巴',
    entry: 'https://campus-talent.alibaba.com/personal/applications',
    loginNote: '阿里统一登录（未登录会跳 SSO/淘宝登录页），登录后落在「我的投递」。接口带 _csrf 时效令牌，直连不可重放 → 页面模式',
    statusHints: ['my', 'apply', 'progress', 'center', 'deliver', 'process', 'application', 'personal'],
    exclude: [TEL],
    risk: '高（登录页 baxia 反爬；_csrf 令牌时效）',
