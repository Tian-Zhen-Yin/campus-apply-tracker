...
function kickMailPoll() {
  ...
  pollMail({})
    .then((r) => {
      ...
      // 定时查询时人不在控制台前：带时间地点的笔试/面试邀约单独弹系统通知；已关联到岗位的附岗位名
      const lines = enrichMailRows(r.fresh).map((m) => {
        const base = tipLine(m);
        return base ? (m.link ? `${base}（${m.link.job}）` : base) : null;
      }).filter(Boolean);
      if (lines.length) {
        notify('📮 笔试/面试邀约', lines.slice(0, 3).join('；') + (lines.length > 3 ? ` 等 ${lines.length} 条` : ''));
      }
    })
    .catch(() => {})
...
// connectOverCDP 必须带 noDefaults：Chrome 152+ 会拒绝附着时的裸 Browser.setDownloadBehavior
// （不带 browserContextId），跳过 Playwright 的下载托管初始化才能附着成功。
...
async function ensureShared() {
  ...
  const browser = await chromium.connectOverCDP(ws, { noDefaults: true });
...
function buildState() {
  ...
  const archived = readArchived(); // 归档只影响控制台总览展示；状态包/dashboard 不带此层
  return {
    now: Date.now(),
    home: HOME,
    sites: Object.keys(SITES).map((k) => {
      ...
      return {
        ...
        last: out ? {
          at: out.at,
          via: out.via,
          records: applyToRecords(k, out.records).map((r) => ((archived[`${k}|${r.job}|${r.dept || ''}`] || archived[`${k}|${r.job}`]) ? { ...r, archived: true } : r)),
        } : null,
...
async function route(req, res, url) {
  ...
  if (p === '/api/debug/dump') {
    // 调试数据落盘（仅本机；opendoc 元数据可能超过 readBody 的 1MB 上限，这里放宽到 20MB 且不要求 JSON）。
    // 按 Buffer 字节累计（字符串拼接会把多字节字符按 UTF-16 计数）；超限不再累计并回 413，不 destroy 请求（destroy 后 end/error 都可能不触发，Promise 会悬挂）
    const chunks = [];
    let size = 0;
    let over = false;
    const raw = await new Promise((resolve, reject) => {
      req.on('data', (c) => { if (over) return; chunks.push(c); size += c.length; if (size > 20 * 1024 * 1024) { over = true; resolve(null); } });
      req.on('end', () => resolve(over ? null : Buffer.concat(chunks)));
      req.on('error', reject);
    });
    if (raw == null) return json(res, 413, { error: '超过 20MB 上限' });
    const n = String(Date.now());
    const target = path.join(os.tmpdir(), `ats-debug-dump-${n}.json`);
    try { fs.writeFileSync(target, raw); log(`📥 调试数据已接收（${raw.length} 字节）→ ${target}`); } catch (_) { /* 落盘失败不影响响应，数据本就只用于调试 */ }
  ...
  switch (p) {
    ...
    case '/api/records/correct': {
      ...
      const dept = String(body.dept || ''); // 同名岗位多部门时的区分（无部门站点恒为空串）
      if (!SITES[site] || !job) return json(res, 400, { error: '参数缺失（site/job）' });
      try {
        if (body.archive !== undefined) {
          const on = setArchived(site, job, !!body.archive, dept);
          log(on ? `🗄️ ${SITES[site].label}「${job}${dept ? `·${dept}` : ''}」已归档——总览隐藏，站点明细里可取消` : `📤 ${SITES[site].label}「${job}${dept ? `·${dept}` : ''}」已取消归档`);
          return json(res, 200, { ok: true, archived: on });
        }
        if (body.reset) {
          setCorrection(site, job, null, dept);
          log(`✏️ ${SITES[site].label}「${job}${dept ? `·${dept}` : ''}」已恢复官网状态`);
        } else {
          if (!isValidStatus(body.status)) return json(res, 400, { error: '未知状态码' });
          const fix = setCorrection(site, job, String(body.status), dept);
          log(`✏️ ${SITES[site].label}「${job}${dept ? `·${dept}` : ''}」人工修正为「${fix.statusRaw}」`);
        }
    ...
    case '/api/mail/link': {
      // 人工绑定/解绑 邮件↔投递记录（ADR-0003：只挂载；自动匹配无歧义时已自行成立）
      const id = String(body.id || '');
      if (!id) return json(res, 400, { error: '需要邮件 id' });
      try {
        if (body.reset) {
          unbindMail(id);
          log(`🔗 已取消邮件人工关联（${id}）`);
        ...
        const target = {
          kind: body.kind === 'apps' ? 'apps' : 'site',
          site: String(body.site || ''),
          company: String(body.company || ''),
          job: String(body.job || ''),
          dept: String(body.dept || ''),
        };
        if (!target.job) return json(res, 400, { error: '需要目标岗位' });
        if (target.kind === 'site' && !SITES[target.site]) return json(res, 400, { error: '未知站点' });
        bindMail(id, target);
        log(`🔗 邮件已关联：${target.kind === 'site' ? (SITES[target.site] || {}).label || target.site : target.company}「${target.job}${target.dept ? `·${target.dept}` : ''}」`);
        return json(res, 200, { ok: true });
      } catch (e) { return json(res, 400, { error: String(e?.message || e) }); }
    }
    case '/api/report': {
