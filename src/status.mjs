...
import { extractApplications, recordKey } from './extract.mjs';
...
export async function status(site, opts = {}) {
  ...
  try {
    let text = null, via = null, authFail = false, pageRecords = null;
    const probes = [];
    for (const spec of candidates) {
      const r = await trySpec(ctx, cfg, spec, opts);
      probes.push(...(r.probes || []));
      if (r.text) { text = r.text; via = r.via; pageRecords = r.records || null; break; }
    ...
    const ex = pageRecords ? { records: pageRecords, ok: true, reason: 'ok' } : extractApplications(text);
    ...
    appendJsonl(historyFile(site), { at: stamp, records: ex.records.map(({ job, dept, statusRaw, status }) => ({ job, dept: dept ?? '', statusRaw, status })) });
    ...
    const changes = ex.records.filter((r) => prev.has(recordKey(r)) && prev.get(recordKey(r)) !== r.statusRaw);
    if (changes.length) {
      console.log('\n状态变化:');
      for (const c of changes) console.log(`  ${c.job}${c.dept ? `（${c.dept}）` : ''}: ${prev.get(recordKey(c))} → ${c.statusRaw}`);
    }
...
// 页面模式多命中合并：从最新命中往回并（SPA 翻页/重复加载同一接口会产生多份，只取最新可能漏页），
// 新记录排前、去重保首个 → 同一记录保留最新状态；bestText 也取最新有效命中（旧的携带过期状态）
export function mergePageHits(hits, extract = extractApplications) {
  let merged = [];
  let bestText = null;
  for (let i = hits.length - 1; i >= 0; i--) {
    const ex = extract(hits[i]);
    if (!ex.records.length) continue;
    if (!bestText) bestText = hits[i];
    merged = merged.concat(ex.records);
  }
  const seen = new Set();
...
async function trySpec(ctx, cfg, spec, opts) {
  ...
  if (hits.length) {
    // 逐个 hit 独立解析再合并（SPA 翻页/重复加载同一接口会产生多个命中，取最新一个可能漏页）；
    // 拦截到的全部解不出记录时（Moka 等加密返回）才用页内 JSON.parse 钩子兜底——
    // 明文接口绝不能走钩子：页面上更晚的 JSON.parse 碎片（单条记录、流程步骤）会冒充完整数据
    const merged = mergePageHits(hits);
    if (merged.records.length) {
      return { text: merged.text, records: merged.records, via: 'page', probes };
    }
    return { text: await tryHookedDecrypted(page, hits[hits.length - 1], probes), via: 'page', probes };
  }
...
function lastHistory(site) {
  ...
  try {
    ...
    return new Map(last.records.map((r) => [recordKey(r), r.statusRaw]));
