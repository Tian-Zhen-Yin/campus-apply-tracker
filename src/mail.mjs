...
// 邮件行稳定 id：内容哈希（mail.jsonl 历史行没有 id，关联绑定用它对上同一封）
export function mailId(row) {
  const s = `${row.at}|${row.from || ''}|${row.subject}`;
  let h = 2166136261;
...
// 邀约行 → 系统通知用的单行摘要（无 tip 返回 null）：`快手 面试·二面 09-10 14:30 腾讯会议`
export function tipLine(row) {
  if (!row || !row.tip) return null;
  const t = row.tip;
  const parts = [t.round && t.round !== t.kind ? `${t.kind}·${t.round}` : t.kind];
  const when = String(t.iso || '').slice(5).replace('T', ' '); // 2026-09-10T14:30 → 09-10 14:30
  if (when) parts.push(when);
  if (t.place) parts.push(t.place);
  return `${row.company || '未知来源'} ${parts.join(' ')}`;
}
...
async function pollAccount(account, { days, limit }) {
  ...
  for (const { uid, ...entry } of freshRows) {
    fs.appendFileSync(mailLogFile(), JSON.stringify({ id: mailId(entry), ...entry }) + '\n');
  }
