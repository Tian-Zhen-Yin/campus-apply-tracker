...
const NAME_KEYS = [/^(jobName|positionName|postName|name|title|position)$/i, /(jobName|positionName|postName)/i, /^(jobTitle|positionTitle)$/i];
...
// 毫秒/秒级时间戳 → 本地 'YYYY-MM-DD HH:mm'，其余原样返回
const fmtTime = (v) => {
  const s = String(v ?? '');
  if (!/^\d{12,13}$/.test(s)) return s;
  const d = new Date(/^\d{13}$/.test(s) ? +s : +s * 1000);
  if (isNaN(d)) return s;
...
// 记录身份键：岗位+部门（同名岗位可投不同部门，阿里校招）。抽取去重/状态对比/时间线/覆盖层共用
export const recordKey = (r) => `${r.job}|${r.dept ?? ''}`;
...
export function extractApplications(jsonText) {
  ...
  for (const it of best) {
    ...
    // 同名岗位可在不同部门重复投递（阿里校招），按 岗位+部门 去重而不是仅岗位
    const key = recordKey({ job, dept });
    ...
    records.push({
      ...
      appliedAt: appliedAt == null ? '' : fmtTime(appliedAt),
