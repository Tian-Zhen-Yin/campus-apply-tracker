...
- **岗位库**:岗位源可多个——**原厂岗位包**(默认,开箱即用:点「同步岗位库」自动从公开仓库拉最新策展岗位,更新尽力而为,通常每工作日 12:00 / 21:30 各一次;离线回落安装包内置版本);你也可以**添加自己的腾讯文档智能表格**作为文档源(需「有链接即可查看」,试读预览确认后入库,手动分源同步,可随时移除)。浏览、搜索、截止提醒、「已投/不投/关注」本地标记(重同步不丢),官网已有投递的岗位会提示「可能已投」一键确认;
...
**岗位收录助手**(浏览器插件,可选):装到 Chrome/Edge 后在任意岗位详情页点图标,自动识别公司/岗位/城市——确认收录时**默认同时记为已投递**(岗位库条目+投递总览记录一次完成;只是先收藏还没投,取消勾选即可)。可选开启**页面悬浮球**:岗位页右下角自动浮现小球,点开即收录,免找工具栏;该开关走二次授权(`optional_host_permissions`),仅本机判断是否岗位页,不外发数据,关闭即撤回。识别在本页完成,结果只发到本机 127.0.0.1:7788,不经任何服务器。源码在 [plugin/](plugin/),下载 zip 见 Releases(解压后 chrome://extensions 开发者模式加载)。
